...
1. **Минимизируй пайпы** — jq/ugrep/fd делают всё сами:
   ```bash
   # ❌ fd -e json | xargs cat | jq '.x' | sort | uniq -c
   # ✅ jq -s '[.[].x] | group_by(.) | map({v:.[0], n:length})' *.json
   ```
2. **fd -x вместо xargs** — встроенный параллелизм и обработка пробелов
3. **Preview перед деструктивом** — `sd -p`, `fd ... ` без `-x rm` сначала
4. **Пробелы в именах** — `fd -x cmd {}` или `fd -0 | xargs -0`
