...
keymapN(
  ...
  function()
    vim.api.nvim_exec2(
      'silent! /\\<\\(' ..
        table.concat(
          {'а', 'в', 'во', 'за', 'из', 'к', 'на', 'не', 'о', 'об', 'от', 'по', 'с', 'со', 'у', 'без', 'для', 'до', 'между', 'над', 'перед', 'под', 'как', 'после', 'при', 'через', 'вдоль', 'вместо', 'благодаря', 'возле', 'вокруг', 'из-за', 'из-под', 'кроме', 'мимо', 'и', 'около', 'прежде', 'про', 'против', 'ради', 'сквозь', 'спустя', 'среди'},
          '\\|')
        .. '\\)\\>\\zs\\s\\|\\s\\ze—',
      { output = false })
  end,
