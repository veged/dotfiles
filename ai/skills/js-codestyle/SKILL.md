...
- Если в репозитории уже есть более сильная норма через линтер, форматтер или явные инструкции — она важнее общего гайда.
- Используй гайд как источник решений по стилю, а не как повод переписывать весь соседний код без запроса.
- Если стиль проекта расходится с гайдом точечно, предпочитай консистентность проекта.
- Следи не только за синтаксисом, но и за философией: лаконичность, отсутствие дублирования, прагматизм, производительность.

## Ресурсы

- [`eslint.config.js`](eslint.config.js) — образцовый ESLint flat-конфиг, отражающий правила этого скилла (2 пробела, single quotes, без `;`, `one-var: consecutive`, набор `unicorn`, JS+TS). Используй его как стартовую точку для нового проекта или как источник для подсветки нарушений; при ревью можно сверяться с правилами оттуда.
...
### Подход
- Unit-тесты: писать осмысленно, особенно для AI-генерируемого кода с нетривиальной логикой
- Фреймворк: Node.js testing framework
- TDD vs тесты после: по ситуации, TDD не включать ритуально без явного запроса или понятного регрессионного риска
- Новый автотест должен защищать конкретный поведенческий инвариант или риск регрессии
- Не писать тесты на «возвращение старого API» без явной просьбы сохранить обратную совместимость
- Не закреплять хрупкие детали: точные строки, снапшоты, порядок, номера строк, внутренние имена и текст ошибок проверять только когда это публичный контракт
